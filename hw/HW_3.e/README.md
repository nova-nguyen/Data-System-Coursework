...
**Note that you may complete this homework with one partner.  You must type up all of the code yourself, but you can work together and look at each other's code.  You _must_ include the name of your partner, if any, in the cell at the end of webscraping2.ipynb.**

---

## Step 1: Working with XPath

Complete [`webscraping2.ipynb`](webscraping2.ipynb).  This notebook contains exercises to practice web scraping from an HTML file containing a table.
...
- Use the terminal command `git add <filepath>` for each file that you've changed and want to submit.  For this homework, you'll likely want the following (remember that `$` represents the prompt, so you don't type that part):
    ...
    ```
    $ git add hw/HW_3.e/webscraping2.ipynb
    ```
